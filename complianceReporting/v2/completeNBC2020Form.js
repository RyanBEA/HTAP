...
function createCell(content, width, options = {}) {
  const { bold = false, shading = null, verticalAlign = VerticalAlign.TOP,
          alignment = AlignmentType.LEFT, colSpan = 1, rowSpan = 1,
          italics = false, size = 20 } = options;
...
console.log("Generating comprehensive NBC 2020 form with all 9 validated tables...");
...
// ============================================================================
// DOCUMENT GENERATION - Combine all 9 tables
// ============================================================================
...
const doc = new Document({
  sections: [{
      ...
      default: new Header({
        children: [
          new Paragraph({
            ...
            children: [new TextRun({ text: "NBC 2020, Subsection 9.36", bold: true, size: 24 })]
          }),
          new Paragraph({
            alignment: AlignmentType.RIGHT,
            children: [new TextRun({ text: "Submittal Form", bold: true, size: 24 })]
          })
        ]
      })
    },
    children: [
      ...
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "D.1 Performance Compliance", bold: true, size: 24 })]
      }),
      table4,
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "E. Additional Information", bold: true, size: 24 })]
      }),
      table5,
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "F. Performance Energy Modeling Professional", bold: true, size: 24 })]
      }),
      table6,
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "D.2 Effective Thermal Resistance", bold: true, size: 24 })]
      }),
      table7,
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "G. Tiered Energy Compliance", bold: true, size: 24 })]
      }),
      table8,
      new Paragraph({
        spacing: { before: 240, after: 120 },
        children: [new TextRun({ text: "H. Designer Declaration", bold: true, size: 24 })]
      }),
      table9
    ]
  }]
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("NBC2020_Complete_Form_9Tables.docx", buffer);
  console.log("✓ Complete NBC 2020 form generated: NBC2020_Complete_Form_9Tables.docx");
  console.log("  - 9 tables with validated structures");
  console.log("  - All column/row counts match original");
});
